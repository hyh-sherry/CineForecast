...
max_value <- max(imdb$movie_budget)
...
```{r}
library(lme4)
model.mix1 <- lmer(imdb_score ~ (1|release_month) + (1|release_year) + duration + as.factor(language) + as.factor(country) + as.factor(maturity_rating) + poly(aspect_ratio,2) + nb_news_articles + as.factor(colour_film) + poly(nb_faces,2) + action + adventure + thriller + musical + romance + horror + drama + animation + crime + movie_meter_IMDBpro + biography + comedy + history + family + documentary + plot_topic1 * plot_topic4 + (1 | director) + (1 | actor1)+ (1 | actor2)+ (1 | actor3)+(1|distributor) + (1|cinematographer) + (movie_budget|production_company), data = imdb)
...
train_index <- sample(num_samples,num_samples*0.6)  ##create indexes for my training data
